...
class NameScope extends SQLConfHelper {
  private val planOutputs = new mutable.ArrayBuffer[PlanOutput]
  ...
  def getAllAttributes: Seq[Attribute] = {
    planOutputs.flatMap(_.attributes).toSeq
  }
   ...
   *
   * One example of a query with a star:
   *
   ...
   */
  def expandStar(unresolvedStar: UnresolvedStar): Seq[Attribute] = {
    if (planOutputs.size != 1) {
      throw QueryCompilationErrors.invalidStarUsageError("query", Seq(unresolvedStar))
    }

    planOutputs.head.expandStar(unresolvedStar)
  }

  /**
   * Get all matched attributes by a multipart name.
   *
   ...
   *   - `table.column`
   *   - `column`
   * @return All the attributes matched by the `multipartName`.
   */
  def getMatchedAttributes(multipartName: Seq[String]): Seq[Attribute] = {
    planOutputs.flatMap(planOutput => planOutput.findAttributes(multipartName)).toSeq
  }
...
class PlanOutput(
   ...
   *
   * See [[NameScope.getMatchedAttributes]] for more details.
   *
   * @param multipartName Multipart attribute name.
   * @return Matched attributes or [[Seq.empty]] otherwise.
   */
  def findAttributes(multipartName: Seq[String]): Seq[Attribute] = {
   ...
   */
  def expandStar(unresolvedStar: UnresolvedStar): Seq[Attribute] = {
    val expansionResult = unresolvedStar.expandStar(
    ...
    expansionResult.map(expression => {
      try {
        expression.asInstanceOf[Attribute]
      } catch {
        case ex: Throwable =>
          throw QueryCompilationErrors.unsupportedSinglePassAnalyzerFeature("struct expansion")
      }
    })
  }
}
